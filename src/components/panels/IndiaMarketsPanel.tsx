...
// ── NSE universe loaded from /public/data/nse-sectors.json ────────────────────
// Used to be a 30-line hardcoded constant inside this file.  Now versioned +
// citable, independent of code releases.
...
interface NseFile {
  ...
  generatedAt:   string
  source:        string
  mainIndices:   MainIndex[]
  sectors:       SectorDef[]
  sectorStocks:  Record<string, { sym: string; name: string }[]>
}
const nseFetcher = (url: string) => fetch(url).then(r => r.json() as Promise<NseFile>)
...
export default function IndiaMarketsPanel() {
  const { data: nseFile } = useSWR<NseFile>('/data/nse-sectors.json', nseFetcher, {
    revalidateOnFocus: false, dedupingInterval: 60_000_000,
  })
  const MAIN_INDICES = useMemo<MainIndex[]>(() => nseFile?.mainIndices ?? [], [nseFile])
  const SECTORS = useMemo<SectorDef[]>(() => nseFile?.sectors ?? [], [nseFile])
  const NSE_SECTOR_STOCKS = useMemo(() => nseFile?.sectorStocks ?? {}, [nseFile])
  ...
  // Seed empty rows once the dataset resolves
  useEffect(() => {
    if (MAIN_INDICES.length === 0) return
    setIndices(prev => prev.length ? prev : MAIN_INDICES.map(i => ({ ...i, price: null, change: null })))
    setSectors(prev => prev.length ? prev : SECTORS.map(s => ({ ...s, price: null, change: null })))
  }, [MAIN_INDICES, SECTORS])
    ...
    <div className="panel" style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <div className="panel-header" style={{ justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          <DataQualityBadge kind="live" small tooltip="Yahoo Finance live quotes, polled every 60s" />
          <DataAgeBadge timestamp={lastUpdatedTs} freshSecs={90} staleSecs={300} small />
          {nseFile?.generatedAt && (
            <DataQualityBadge
              kind="versioned"
              small
              tooltip={`NSE sector membership snapshot from ${new Date(nseFile.generatedAt).toLocaleDateString()}. Source: ${nseFile.source}`}
            />
          )}
        </div>
      </div>
